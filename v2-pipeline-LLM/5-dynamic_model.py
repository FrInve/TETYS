...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        prog="TimeSeries Extraction from BERTopic",
    ...
    logging.basicConfig(
        ...
        filename=f"./logs/dynamic_{bins}.log",
        encoding="utf-8",
        level=logging.INFO,
    )
    ...
    try:
        topics_ot.to_parquet(
            f"./data/processed/cortoviz_data/{bins}_dyn_raw.parquet",
    ...
    topics_ot_pivot = (
        topics_ot[["Topic", "Frequency", "Date"]]
        .pivot(index="Date", columns="Topic", values="Frequency")
        .copy(deep=True)
    )

    try:
        topics_ot_pivot.to_parquet(
            f"./data/processed/cortoviz_data/{bins}_dyn_pivot.parquet",
            allow_truncated_timestamps=True,
        )
    except:
        logging.error("Can't export to parquet")
    ...
    topics_ot_pivot_norm = topics_ot_pivot.div(
        topics_ot_pivot.sum(axis=1), axis=0
    ).copy(deep=True)

    try:
        topics_ot_pivot_norm.to_parquet(
            f"./data/processed/cortoviz_data/{bins}_dyn_pivot_norm.parquet",
            allow_truncated_timestamps=True,
        )
    except:
        logging.error("Can't export to parquet")
    topics_ot_pivot_norm.to_csv(
        f"./data/processed/cortoviz_data/{bins}_dyn_pivot_norm.csv"
    )
