...
if __name__ == "__main__":
    ...
    topic_model = BERTopic.load(f'./models/group_{GROUP}/model_group_3_dbcv_0_36', embedding_model="sentence-transformers/allenai-specter")
    ...
    try:
        topics_ot.to_parquet(f'./data/processed/group_{GROUP}/{bins}_dyn_raw.parquet', allow_truncated_timestamps=True)
    except:
        logging.error("Can't export to parquet")
    topics_ot.to_csv(f'./data/processed/group_{GROUP}/{bins}_dyn_raw.csv')
    ...
    try:
        topics_ot_pivot.to_parquet(f'./data/processed/group_{GROUP}/{bins}_dyn_pivot.parquet',allow_truncated_timestamps=True)
    except:
        logging.error("Can't export to parquet")
    topics_ot_pivot.to_csv(f'./data/processed/group_{GROUP}/{bins}_dyn_pivot.csv')
    ...
    try:
        topics_ot_pivot_norm.to_parquet(f'./data/processed/group_{GROUP}/{bins}_dyn_pivot_norm.parquet',allow_truncated_timestamps=True)
    except:
        logging.error("Can't export to parquet")
    topics_ot_pivot_norm.to_csv(f'./data/processed/group_{GROUP}/{bins}_dyn_pivot_norm.csv')
