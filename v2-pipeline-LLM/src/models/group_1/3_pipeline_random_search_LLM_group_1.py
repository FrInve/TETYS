...
if __name__ == '__main__':
    # Set logging
    logging.basicConfig(format='%(asctime)s | %(levelname)s:%(message)s',filename=f'./logs/group_{GROUP}/new_LLM_pipeline_random_search_group_{GROUP}', encoding='utf-8', level=logging.INFO)
    ...
    embed = f'./data/interim/group_{GROUP}/new/{embed_model}'
    ...
    mask = np.random.choice([False, True], len(embeddings), p=[0.97, 0.03])
    ...
    param_grid = {'umap__n_neighbors':[2,20,50,100],
                  'umap__min_dist':[0.0],
    ...
    for params in param_list:
        ...
        if current_score > best_score:
            ...
            if best_score >= 0.42:
                ...
                # model save
                topic_model.save(f'./models/group_{GROUP}/new/random_LLM_model_group_{GROUP}_dbcv_{best_score}_new', save_embedding_model=False)
