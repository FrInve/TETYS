...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(prog="TimeSeries Extraction from BERTopic",
                            description="Extract time series data from a LLM BERTopic model")
    ...
    # Set logging
    logging.basicConfig(format='%(asctime)s | %(levelname)s:%(message)s',filename=f"./logs/group_{GROUP}/dynamic_{bins}_LLM.log", encoding='utf-8', level=logging.INFO)
    ...
    topic_model = BERTopic.load(f'./models/group_{GROUP}/random_LLM_model_group_1_dbcv_0_52', embedding_model="Salesforce/SFR-Embedding-2_R")
    ...
    try:
        topics_ot.to_parquet(f'./data/processed/group_{GROUP}/timeseries/LLM/LLM_{bins}_dyn_raw.parquet', allow_truncated_timestamps=True)
    except:
        logging.error("Can't export to parquet")
    topics_ot.to_csv(f'./data/processed/group_{GROUP}/timeseries/LLM/LLM_{bins}_dyn_raw.csv')
    ...
    try:
        topics_ot_pivot.to_parquet(f'./data/processed/group_{GROUP}/timeseries/LLM/LLM_{bins}_dyn_pivot.parquet',allow_truncated_timestamps=True)
    except:
        logging.error("Can't export to parquet")
    topics_ot_pivot.to_csv(f'./data/processed/group_{GROUP}/timeseries/LLM/LLM_{bins}_dyn_pivot.csv')
    ...
    try:
        topics_ot_pivot_norm.to_parquet(f'./data/processed/group_{GROUP}/timeseries/LLM/LLM_{bins}_dyn_pivot_norm.parquet',allow_truncated_timestamps=True)
    except:
        logging.error("Can't export to parquet")
    topics_ot_pivot_norm.to_csv(f'./data/processed/group_{GROUP}/timeseries/LLM/LLM_{bins}_dyn_pivot_norm.csv')
