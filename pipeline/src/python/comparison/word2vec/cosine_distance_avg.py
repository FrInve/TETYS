...
if __name__ == "__main__":
    ...
    for row in df_lev.iterrows():
        ...
        avg_sim_single = 0
    #Si calcola la similarità tra ogni parola di tetys per una parola di andrea. Alla fine viene salvata la media
        for i in range(1, len(model_list)):
            sum_sim = 0
            lunghezza = len(andrea_list)
            for j in andrea_list:
                try:
                    sum_sim += model.wv.similarity(model_list[i], j)
                    #print(sum_sim)
                except:
                    if lunghezza > 1:
                        lunghezza -= 1
            
            avg_sim_single += sum_sim / lunghezza
        
        avg_distance = avg_sim_single/(len(model_list))
